<?php
$this->layout('layout', ['title' => 'Users', 'email' => $auth->getEmail()]);
?>
...
<main id="js-page-content" role="main" class="page-content mt-3">
    ...
    <div class="row">
        <div class="col-xl-12">
            <?php if ($auth->hasRole(\Delight\Auth\Role::ADMIN)): ?>
                <a class="btn btn-success" href="/page_create">Добавить</a>
            <?php endif; ?>
    ...
    <div class="row" id="js-contacts">
        ...
        <?php foreach ($users as $user): ?>
            <div class="col-xl-4">
                <div id="c_<?=$user['id']; ?>" class="card border shadow-0 mb-g shadow-sm-hover" data-filter-tags="<?=mb_strtolower($user['name']); ?>">
                    <div class="card-body border-faded border-top-0 border-left-0 border-right-0 rounded-top">
                        <div class="d-flex flex-row align-items-center">
                            <?php $status = 'info'; ?>
                            <?php if($user['status'] == 'online') $status = 'success'; ?>
                            <?php if($user['status'] == 'busy') $status = 'danger'; ?>
                            <?php if($user['status'] == 'away') $status = 'warning';  ?>
                            ...
                            <div class="info-card-text flex-1">
                                <a href="javascript:void(0);" class="fs-xl text-truncate text-truncate-lg text-info" data-toggle="dropdown" aria-expanded="false">
                                   ...
                                   <?php if ($auth->hasRole(\Delight\Auth\Role::ADMIN) or $user['id'] == $auth->getUserId()): ?>
